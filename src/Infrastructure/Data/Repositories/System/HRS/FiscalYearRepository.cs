...
namespace Infrastructure.Data.Repositories.System.HRS
{
    public sealed class FiscalYearRepository : IFiscalYearRepository
    {
        ...
        public FiscalYearRepository(ApplicationDbContext db, ICurrentUser currentUser)
        {
            _db = db;
        ...
        public async Task<List<FiscalYear>> GetByCompanyIdAsync()
        {
        ...
        public async Task<FiscalYear> AddAsync(FiscalYear entity)
        {
            await _db.FiscalYears.AddAsync(entity);
            return entity;
        }

        public Task UpdateAsync(FiscalYear entity)
        {
            _db.FiscalYears.Update(entity);
        ...
        public async Task DeleteAsync(int id)
        {
            ...
            if (item != null) _db.FiscalYears.Remove(item);
        }

        public async Task<bool> ExistsAsync(int id)
            => await _db.FiscalYears.AnyAsync(x => x.Id == id);

        public async Task<bool> CodeExistsAsync(string code)
        {
            if (string.IsNullOrWhiteSpace(code)) return false;
            var trimmedCode = code.Trim();
            return await _db.FiscalYears.AnyAsync(x => x.Code != null && x.Code.Trim() == trimmedCode);
        }
        ...
        public async Task<bool> CodeExistsAsync(string code, int excludeId)
        {
            if (string.IsNullOrWhiteSpace(code)) return false;
            var trimmedCode = code.Trim();
            return await _db.FiscalYears.AnyAsync(x => x.Code != null && x.Code.Trim() == trimmedCode && x.Id != excludeId);
        }

        public async Task<PagedResult<FiscalYear>> GetPagedAsync(int pageNumber, int pageSize, string? searchTerm)
        {
            var companyId = _currentUser.CompanyId;
            pageNumber = pageNumber <= 0 ? 1 : pageNumber;
            pageSize = pageSize <= 0 ? 20 : pageSize;

            IQueryable<FiscalYear> query = _db.FiscalYears
                .Where(x => x.CancelDate == null && x.CompanyId == companyId)
                .Include(x => x.Company)
                .AsNoTracking();
        ...
        {
            var allCodes = await _db.FiscalYears
                .Where(x => x.CancelDate == null && x.CompanyId == companyId)
                .Select(x => x.Code)
                .ToListAsync(ct);
