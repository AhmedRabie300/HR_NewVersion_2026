...
namespace Infrastructure.Data.Repositories.System.MasterData
{
    public sealed class RegionRepository : IRegionRepository
    {
        ...
        public async Task<Region?> GetByCodeAsync(string code)
            => await _db.Regions.FirstOrDefaultAsync(x => x.Code == code);
        ...
        public async Task<List<Region>> GetByCountryIdAsync(int countryId)
            => await _db.Regions
                .Where(x => x.CancelDate == null && x.CountryId == countryId)
                .OrderBy(x => x.Code)
                .AsNoTracking()
                .ToListAsync();

        public async Task<List<Region>> GetByCompanyIdAsync(int companyId)
            => await _db.Regions
        ...
        {
            var item = await _db.Regions.FindAsync(id);
            if (item != null) _db.Regions.Remove(item);
        }

        public async Task<bool> ExistsAsync(int id)
            => await _db.Regions.AnyAsync(x => x.Id == id);

        public async Task<bool> CodeExistsAsync(string code)
            => await _db.Regions.AnyAsync(x => x.Code == code);

        public async Task<bool> CodeExistsAsync(string code, int excludeId)
            => await _db.Regions.AnyAsync(x => x.Code == code && x.Id != excludeId);

        public async Task<PagedResult<Region>> GetPagedAsync(int pageNumber, int pageSize, string? searchTerm)
        {
            ...
            return new PagedResult<Region>(items, pageNumber, pageSize, totalCount);
        }
