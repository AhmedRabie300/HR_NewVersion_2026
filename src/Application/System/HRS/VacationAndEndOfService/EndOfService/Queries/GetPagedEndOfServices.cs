...
{
    public static class GetPagedEndOfServices
    {
        public record Query(
            int PageNumber = 1,
            int PageSize = 20,
            string? SearchTerm = null
        ) : IRequest<PagedResult<EndOfServiceDto>>;

        public class Handler : IRequestHandler<Query, PagedResult<EndOfServiceDto>>
        {
            ...
            public async Task<PagedResult<EndOfServiceDto>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var items = pagedResult.Items.Select(x => new EndOfServiceDto(
                    Id: x.Id,
                    Code: x.Code,
                    EngName: x.EngName,
                    ArbName: x.ArbName,
                ...
                return new PagedResult<EndOfServiceDto>(
                    items,
