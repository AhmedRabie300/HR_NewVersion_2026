...
{
    public static class GetPagedFiscalYearPeriods
    {
        public record Query(
            int PageNumber = 1,
            int PageSize = 20,
            string? SearchTerm = null
        ) : IRequest<PagedResult<FiscalYearPeriodDto>>;

        public class Handler : IRequestHandler<Query, PagedResult<FiscalYearPeriodDto>>
        {
            ...
            public async Task<PagedResult<FiscalYearPeriodDto>> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var items = pagedResult.Items.Select(x => new FiscalYearPeriodDto(
                    Id: x.Id,
                    Code: x.Code,
                ...
                return new PagedResult<FiscalYearPeriodDto>(
                    items,
